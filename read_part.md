...
-----------------
```
从 "Electronic Transition Derivatives"开始，到空行结束中间的数据是我们要处理的区域，其中每一行为共有以空格分隔的数据若干，首先你需要对每一行进行分割，并将所有的'D'换成'e'，然后将第一个元素是2的行的行的数据整合到一个数组中（每行开头的2不需要加入），同样的方法得到第一个元素是3的所有行的数据，以及第一个是4的数据，你会发现这三个数组长度都为3N+3，将这三个数组拼为一个(3N+3)\*3的矩阵，除去前三行，输出剩下的3N\*3的矩阵
