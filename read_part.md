...
-----------------
```
从 "Electronic Transition Derivatives"开始，到空行结束中间的数据是我们要处理的区域（不包括开头和结束行），其中每一行为共有以空格分隔的数据若干，首先你需要对每一行进行分割，并将所有的'D'换成'e'，然后将第一个元素是2的行的数据整合到一个数组中（每行开头的数字不需要加入）记为a，a的长度为3N+3;将第一个元素是3的行的数据整合到一个数组中（每行开头的数字不需要加入）记为b，b的长度为3N+3;然后将第一个元素是4的行的数据整合到一个数组中（每行开头的数字不需要加入）记为c，c的长度为3N+3;。将a,b,c拼为一个(3N+3)\*3的矩阵，除去前三行，输出剩下的3N\*3的矩阵
