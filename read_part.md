...
|get_mode|file_name: str : 输入文件名 <br> N: int : 原子数量|shape=(3N-6)\*N\*3的数组|从 gaussian log 中读取振动模式的分量|
...
...
```
你需要找到所有' Frequencies --- '所在的行，其后面的第5到3N+4行（闭区间）的数据是我们需要处理的区域，这样的区域是3N行若干列（最少4列，最多8列）的数据，同一行的数据以空格分割，你需要将该区域的数据转化为array，并将所有的这样的区域的array去掉前三列并通过hstack拼起来，最后得到一个3N行3N-6列的array，将这个array转置并reshape为(3N-6)\*N\*3的数组并输出
